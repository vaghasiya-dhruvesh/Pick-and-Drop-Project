...
if(isset($_POST['submit'])){
    $search=$_POST['search'];
    include '../dbcon.php';
    $qry="SELECT * FROM vehicle where vname LIKE '%$search%' OR brand LIKE '%$search%' OR ftype LIKE '%$search%' OR color LIKE '%$search%' OR price LIKE '%$search%' OR seat LIKE '%$search%'";
...
if(isset($_POST['stt'])){
    $status=$_POST['status'];
    include '../dbcon.php';
    $qry="SELECT * FROM vehicle where status LIKE '%$status%'";
    $cmd1=mysqli_query($con,$qry);
    if($cmd1)
    {
        while($row=mysqli_fetch_array($cmd1)){
